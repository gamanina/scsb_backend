...
@Controller
@RequestMapping("/businessAd")
public class BusinessAdController {
	...
	@Autowired
	private BusinessAdValidator formValidator;
	
	@Value("${web.upload-path}")
	private String uploadPath;
	@Value("${business.ad.image}")
	...
	protected final String rightsString = Constants.PAGE_KEY_BUSINESS_AD;
	protected final String sheetType = Constants.BUSINESS_AD_SHEET_TYPE;
	protected final String reFlieName = "businessAd";
	protected final String viewAddUrl = "views/businessAd/add";
	
	private void setCommon(Model model, HttpServletRequest request, BusinessAdForm form) throws Exception
	{
		// TODO 刪除
		if(isTest)
		{
	...
	@RequestMapping("/add")
	public String add(Model model, HttpServletRequest request, @ModelAttribute BusinessAdForm form) 
	{
		try 
		{
			// 檢查權限
	...
	@RequestMapping("/save")
	public String save(Model model, HttpServletRequest request, @ModelAttribute BusinessAdForm form, BindingResult bindingResult) 
	{
		try 
		{
			Manager manager = (Manager) request.getSession().getAttribute(Constants.SESSION_MEMBER_KEY);
