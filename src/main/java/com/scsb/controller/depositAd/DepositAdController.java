package com.scsb.controller.depositAd;
...
@Controller
@RequestMapping("/depositAd")
public class DepositAdController {
	...
	@Autowired
	private DepositAdValidator formValidator;
	
	@Value("${web.upload-path}")
	private String uploadPath;
	@Value("${deposit.ad.image}")
	...
	protected final String rightsString = Constants.PAGE_KEY_DEPOSIT_AD;
	protected final String sheetType = Constants.DEPOSIT_AD_SHEET_TYPE;
	protected final String reFlieName = "depositAd";
	protected final String viewAddUrl = "views/depositAd/add";
	
	private void setCommon(Model model, HttpServletRequest request, DepositAdForm form) throws Exception
	{
		// TODO 刪除
		if(isTest)
		{
	...
	@RequestMapping("/add")
	public String add(Model model, HttpServletRequest request, @ModelAttribute DepositAdForm form) 
	{
		try 
		{
			// 檢查權限
	...
	@RequestMapping("/save")
	public String save(Model model, HttpServletRequest request, @ModelAttribute DepositAdForm form, BindingResult bindingResult) 
	{
		try 
		{
			Manager manager = (Manager) request.getSession().getAttribute(Constants.SESSION_MEMBER_KEY);
