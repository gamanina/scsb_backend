package com.scsb.controller.cardAd;
...
public class CardAdController {
	...
	@Autowired
	private CardAdValidator formValidator;
	...
	protected final String rightsString = Constants.PAGE_KEY_CARD_AD;
	protected final String sheetType = Constants.CARD_AD_SHEET_TYPE;
	protected final String reFlieName = "cardAd";
	protected final String viewAddUrl = "views/card/ad/add";
	
	private void setCommon(Model model, HttpServletRequest request, CardAdForm form) throws Exception
	{
		// TODO 刪除
		if(isTest)
		{
	...
	@RequestMapping("/add")
	public String add(Model model, HttpServletRequest request, @ModelAttribute CardAdForm form) 
	{
		try 
		{
			// 檢查權限
	...
	@RequestMapping("/save")
	public String save(Model model, HttpServletRequest request, @ModelAttribute CardAdForm form, BindingResult bindingResult) 
	{
		try 
		{
			Manager manager = (Manager) request.getSession().getAttribute(Constants.SESSION_MEMBER_KEY);
