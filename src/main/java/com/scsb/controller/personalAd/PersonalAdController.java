package com.scsb.controller.personalAd;
...
@Controller
@RequestMapping("/personalAd")
public class PersonalAdController {
	...
	@Autowired
	private PersonalAdValidator formValidator;
	
	@Value("${web.upload-path}")
	private String uploadPath;
	@Value("${personal.ad.image}")
	...
	protected final String rightsString = Constants.PAGE_KEY_PERSONAL_AD;
	protected final String sheetType = Constants.PERSONAL_AD_SHEET_TYPE;
	protected final String reFlieName = "personalAd";
	protected final String viewAddUrl = "views/personalAd/add";
	
	private void setCommon(Model model, HttpServletRequest request, PersonalAdForm form) throws Exception
	{
		// TODO 刪除
		if(isTest)
		{
	...
	@RequestMapping("/add")
	public String add(Model model, HttpServletRequest request, @ModelAttribute PersonalAdForm form) 
	{
		try 
		{
			// 檢查權限
	...
	@RequestMapping("/save")
	public String save(Model model, HttpServletRequest request, @ModelAttribute PersonalAdForm form, BindingResult bindingResult) 
	{
		try 
		{
			Manager manager = (Manager) request.getSession().getAttribute(Constants.SESSION_MEMBER_KEY);
