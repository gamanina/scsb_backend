package com.scsb.controller.businessBanner;
...
public class BusinessBannerController {
	...
	@Autowired
	private BusinessBannerValidator formValidator;
	...
	protected final String rightsString = Constants.PAGE_KEY_BUSINESS_BANNER;
	protected final String sheetType = Constants.BUSINESS_BANNER_SHEET_TYPE;
	protected final String reFlieName = "businessBanner";
	protected final String reFlieName2 = "businessBannerMobile";
	protected final String viewAddUrl = "views/businessBanner/add";
	
	private void setCommon(Model model, HttpServletRequest request, BusinessBannerForm form) throws Exception
	{
		// TODO 刪除
		if(isTest)
		{
	...
	@RequestMapping("/add")
	 public String businessBanner(@ModelAttribute BusinessBannerForm form,Model model, HttpServletRequest request)
	{
		try 
		{
			// 檢查權限
	...
	@RequestMapping("/save")
	public String save(Model model, HttpServletRequest request, @ModelAttribute BusinessBannerForm form, BindingResult bindingResult) 
	{
		try 
		{
			Manager manager = (Manager) request.getSession().getAttribute(Constants.SESSION_MEMBER_KEY);
