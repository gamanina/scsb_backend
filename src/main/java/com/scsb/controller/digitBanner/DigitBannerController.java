package com.scsb.controller.digitBanner;
...
@Controller
@RequestMapping("/digitBanner")
public class DigitBannerController {
	...
	@Autowired
	private DigitBannerValidator formValidator;
	
	@Value("${web.upload-path}")
	private String uploadPath;
	@Value("${digit.banner.image}")
	...
	protected final String rightsString = Constants.PAGE_KEY_DIGIT_BANNER;
	protected final String sheetType = Constants.DIGIT_BANNER_SHEET_TYPE;
	protected final String reFlieName = "digitBanner";
	protected final String reFlieName2 = "digitBannerMobile";
	protected final String viewAddUrl = "views/digitBanner/add";
	
	private void setCommon(Model model, HttpServletRequest request, DigitBannerForm form) throws Exception
	{
		// TODO 刪除
		if(isTest)
		{
	...
	@RequestMapping("/add")
	public String add(Model model, HttpServletRequest request, @ModelAttribute DigitBannerForm form) 
	{
		try 
		{
			// 檢查權限
	...
	@RequestMapping("/save")
	public String save(Model model, HttpServletRequest request, @ModelAttribute DigitBannerForm form, BindingResult bindingResult) 
	{
		try 
		{
			Manager manager = (Manager) request.getSession().getAttribute(Constants.SESSION_MEMBER_KEY);
