package com.scsb.controller.personalBanner;
...
@Controller
@RequestMapping("/personalBanner")
public class PersonalBannerController {
	...
	@Autowired
	private PersonalBannerValidator formValidator;
	
	@Value("${web.upload-path}")
	private String uploadPath;
	@Value("${personal.banner.image}")
	...
	protected final String rightsString = Constants.PAGE_KEY_PERSONAL_BANNER;
	protected final String sheetType = Constants.PERSONAL_BANNER_SHEET_TYPE;
	protected final String reFlieName = "personalBanner";
	protected final String reFlieName2 = "personalBannerMobile";
	protected final String viewAddUrl = "views/personalBanner/add";
	
	private void setCommon(Model model, HttpServletRequest request, PersonalBannerForm form) throws Exception
	{
		// TODO 刪除
		if(isTest)
		{
	...
	@RequestMapping("/add")
	public String add(Model model, HttpServletRequest request, @ModelAttribute PersonalBannerForm form) 
	{
		try 
		{
			// 檢查權限
	...
	@RequestMapping("/save")
	public String save(Model model, HttpServletRequest request, @ModelAttribute PersonalBannerForm form, BindingResult bindingResult) 
	{
		try 
		{
			Manager manager = (Manager) request.getSession().getAttribute(Constants.SESSION_MEMBER_KEY);
