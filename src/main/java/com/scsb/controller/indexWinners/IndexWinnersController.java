...
@Controller
@RequestMapping("/indexWinners")
public class IndexWinnersController {
	...
	@Autowired
	private IndexWinnersValidator formValidator;
	...
	protected final String rightsString = Constants.PAGE_KEY_INDEX_WINNERS;
	protected final String sheetType = Constants.INDEX_WINNERS_SHEET_TYPE;
	protected final String reFlieName = "indexWinners";
	protected final String viewAddUrl = "views/indexWinners/add";
	
	private void setCommon(Model model, HttpServletRequest request, IndexWinnersForm form) throws Exception
	{
		// TODO 刪除
		if(isTest)
		{
	...
	@RequestMapping("/add")
	public String add(Model model, HttpServletRequest request, @ModelAttribute IndexWinnersForm form) 
	{
		try 
		{
			// 檢查權限
	...
	@RequestMapping("/save")
	public String save(Model model, HttpServletRequest request, @ModelAttribute IndexWinnersForm form, BindingResult bindingResult) 
	{
		try 
		{
			Manager manager = (Manager) request.getSession().getAttribute(Constants.SESSION_MEMBER_KEY);
