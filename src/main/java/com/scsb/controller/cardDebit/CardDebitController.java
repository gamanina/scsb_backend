...
public class CardDebitController {
	...
	@Autowired
	private CardDebitValidator formValidator;
...
//	private DataOption dataOption;
	...
	@Value("${card.debit.discount.image}")
	...
	protected final String rightsString = Constants.PAGE_KEY_CARD_DEBIT_DISCOUNT;
	protected final String sheetType = Constants.CARD_DEBIT_DISCOUNT_SHEET_TYPE;
	protected final String reFlieName = "cardDebit";
	protected final String viewAddUrl = "views/card/debit/add";

	private void setCommon(Model model, HttpServletRequest request, CardDebitForm form) throws Exception
	{
		// TODO 刪除
		if(isTest)
		{
...
//			model.addAttribute("categoryMap", categoryMap);
	...
	@RequestMapping("/add")
	public String add(Model model, HttpServletRequest request, @ModelAttribute CardDebitForm form) 
	{
		try 
		{
			// 檢查權限
	...
	@RequestMapping("/save")
	public String save(Model model, HttpServletRequest request, @ModelAttribute CardDebitForm form, BindingResult bindingResult) 
	{
		try 
		{
			Manager manager = (Manager) request.getSession().getAttribute(Constants.SESSION_MEMBER_KEY);
