package com.scsb.controller.cardBanner;
...
@Controller
@RequestMapping("/card/banner")
public class CardBannerController {
	...
	@Autowired
	private CardBannerValidator formValidator;
	
	@Value("${web.upload-path}")
	private String uploadPath;
	@Value("${card.banner.image}")
	...
	protected final String rightsString = Constants.PAGE_KEY_CARD_BANNER;
	protected final String sheetType = Constants.CARD_BANNER_SHEET_TYPE;
	protected final String reFlieName = "cardBanner";
	protected final String reFlieName2 = "cardBannerMobile";
	protected final String viewAddUrl = "views/card/banner/add";
	// 預設廣告輪播刊登天數
	protected final int publishedDays = 14;
	
	private void setCommon(Model model, HttpServletRequest request, CardBannerForm form, IndexBannerSetting ibs) throws Exception
	{
		try
		{
			// 審核人
...
//	        }
	...
	@RequestMapping("/add")
	public String add(Model model, HttpServletRequest request, @ModelAttribute CardBannerForm form) 
	{
		try 
		{
			// 檢查權限
	...
	@RequestMapping("/save")
	public String save(Model model, HttpServletRequest request, @ModelAttribute CardBannerForm form, BindingResult bindingResult) 
	{
		try 	
		{
			// 檢查權限
