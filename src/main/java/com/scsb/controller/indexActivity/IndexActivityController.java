package com.scsb.controller.indexActivity;
...
public class IndexActivityController {
	...
	@Autowired
	private IndexActivityValidator formValidator;
	
	@Value("${web.upload-path}")
	private String uploadPath;
	@Value("${index.activity.image}")
	private String imageFolder;
	@Value("${index.activity.file}")
	...
	protected final String rightsString = Constants.PAGE_KEY_INDEX_ACTIVITY;
	protected final String sheetType = Constants.INDEX_ACTIVITY_SHEET_TYPE;
	protected final String reFlieName = "indexActivity";
	protected final String viewAddUrl = "views/indexActivity/add";
	
	private void setCommon(Model model, HttpServletRequest request, IndexActivityForm form) throws Exception
	{
		// TODO 刪除
		if(isTest)
		{
	...
	@RequestMapping("/add")
	public String add(Model model, HttpServletRequest request, @ModelAttribute IndexActivityForm form) 
	{
		try 
		{
			// 檢查權限
	...
	@RequestMapping("/save")
	public String save(Model model, HttpServletRequest request, @ModelAttribute IndexActivityForm form, BindingResult bindingResult) 
	{
		System.out.println("測試Save");
		System.out.println(form.getAgentId());
		try 
		{
			Manager manager = (Manager) request.getSession().getAttribute(Constants.SESSION_MEMBER_KEY);
			// 檢查權限
