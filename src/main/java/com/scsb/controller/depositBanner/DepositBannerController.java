package com.scsb.controller.depositBanner;
...
@Controller
@RequestMapping("/depositBanner")
public class DepositBannerController {
	...
	@Autowired
	private DepositBannerValidator formValidator;
	
	@Value("${web.upload-path}")
	private String uploadPath;
	@Value("${deposit.banner.image}")
	...
	protected final String rightsString = Constants.PAGE_KEY_DEPOSIT_BANNER;
	protected final String sheetType = Constants.DEPOSIT_BANNER_SHEET_TYPE;
	protected final String reFlieName = "depositBanner";
	protected final String reFlieName2 = "depositBannerMobile";
	protected final String viewAddUrl = "views/depositBanner/add";
	
	private void setCommon(Model model, HttpServletRequest request, DepositBannerForm form) throws Exception
	{
		// TODO 刪除
		if(isTest)
		{
	...
	@RequestMapping("/add")
	public String add(Model model, HttpServletRequest request, @ModelAttribute DepositBannerForm form) 
	{
		try 
		{
			// 檢查權限
	...
	@RequestMapping("/save")
	public String save(Model model, HttpServletRequest request, @ModelAttribute DepositBannerForm form, BindingResult bindingResult) 
	{
		try 
		{
			Manager manager = (Manager) request.getSession().getAttribute(Constants.SESSION_MEMBER_KEY);
